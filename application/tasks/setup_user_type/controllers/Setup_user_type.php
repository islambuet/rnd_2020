...
class Setup_user_type extends Root_Controller
{
    ...
    */
    ...
    {
        $items=Query_helper::get_info(TABLE_RND_SETUP_USER_TYPE,array('id','name','remarks','status','ordering'),array('status !="'.SYSTEM_STATUS_DELETE.'"'));
    ...
    public function system_edit($id=0)
    {
        if(isset($this->permissions['action2']) && ($this->permissions['action2']==1))
        {
            ...
            $data['item']=Query_helper::get_info(TABLE_RND_SETUP_USER_TYPE,'*',array('id ='.$item_id),1);
            if(!$data['item'])
            {
    ...
    public function system_save_add_edit()
    {
...

        {
            ...
            if($id>0)
            {
                ...
                Query_helper::update(TABLE_RND_SETUP_USER_TYPE,$data,array('id='.$id));
            }
            else
            {
                $data['user_created']=$user->user_id;
                $data['date_created']=$time;
                Query_helper::add(TABLE_RND_SETUP_USER_TYPE,$data);
            }
