...
class Setup_principal extends Root_Controller
{
    ...
    */
    ...
    public function get_jqx_items($method)
    {
        ...
        if($method=='system_list')
        {
            ...
            $data['code']= array('text'=>$this->lang->line('LABEL_CODE'),'type'=>'string','preference'=>1,'jqx_column'=>true,'column_attributes'=>array('width'=>'"100"'));
    ...
    {
        $items=Query_helper::get_info(TABLE_RND_SETUP_PRINCIPAL,'*',array('status !="'.SYSTEM_STATUS_DELETE.'"'));
    ...
    public function system_add()
    {
        ...
        {
            $data['item']=array();
            $table_fields = $this->db->field_data(TABLE_RND_SETUP_PRINCIPAL);
    ...
    public function system_edit($id=0)
    {
        if(isset($this->permissions['action2']) && ($this->permissions['action2']==1))
        {
            ...
            $data['item']=Query_helper::get_info(TABLE_RND_SETUP_PRINCIPAL,'*',array('id ='.$item_id),1);
            if(!$data['item'])
            {
    ...
    public function system_save_add_edit()
    {
...

        {
            ...
            if($id>0)
            {
                ...
                Query_helper::update(TABLE_RND_SETUP_PRINCIPAL,$data,array('id='.$id));
            }
            else
            {
                $data['user_created']=$user->id;
                $data['date_created']=$time;
                Query_helper::add(TABLE_RND_SETUP_PRINCIPAL,$data);
            }
